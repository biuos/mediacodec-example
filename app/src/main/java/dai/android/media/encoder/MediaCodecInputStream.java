...
public class MediaCodecInputStream extends InputStream {
        ...
        if (mBuffer == null) {
            while (!Thread.interrupted() && !mClosed) {
                synchronized (mMediaCodec) {
                    ...
                    Log.d(TAG, "Index: " + encoderStatus + " Time: " + mBufferInfo.presentationTimeUs + " size: " + mBufferInfo.size);
                    if (encoderStatus >= 0) {
    ...
    public static void readAll(MediaCodecInputStream is,
                               byte[] buffer,
        ...
        do {
            try {
                ...
                onReadAllCallback.onReadOnce(buffer, readSize, copyBufferInfo(is.getLastBufferInfo()));
            } catch (IOException e) {
                e.printStackTrace();
                return;
            }
